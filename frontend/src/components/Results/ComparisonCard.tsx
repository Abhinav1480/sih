  ...
  return (
    <div className="maritime-card p-5 space-y-4">
      <div className="flex items-center justify-between border-b border-white/[0.06] pb-3">
        <div>
          ...
          <div className="font-display font-bold text-lg text-white flex items-center gap-2 mt-0.5">
            ...
            <span className="text-cyan-400 font-normal text-sm font-mono">vs</span>
      ...
      <div className="p-3.5 rounded-xl bg-gradient-to-r from-blue-950/30 to-cyan-950/20 border border-cyan-500/25 text-xs leading-relaxed text-slate-200 flex items-start gap-2.5">
        ...
        <div>
          <div className="font-bold text-cyan-400 mb-0.5 font-mono uppercase text-[10px]">Comparative Verdict</div>
          ...
          {data.metrics.map((m, idx) => (
            ...
            >
              <div className="flex items-center justify-between font-medium">
                ...
                <span className="text-[10px] px-2 py-0.5 rounded bg-orca-panel border border-white/[0.08] text-cyan-400 font-mono font-semibold">
                  {m.favorability}
                </span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 pt-1 border-t border-white/[0.06]">
                <div>
                  <span className="text-[10px] text-slate-400 block font-mono">{data.location_a.name}</span>
                  <span className="font-bold text-sm text-white font-mono">
                    {m.location_a_value} <span className="text-[10px] font-normal text-slate-400">{m.unit}</span>
                  </span>
                </div>
                <div>
                  <span className="text-[10px] text-slate-400 block font-mono">{data.location_b.name}</span>
                  <span className="font-bold text-sm text-white font-mono">
